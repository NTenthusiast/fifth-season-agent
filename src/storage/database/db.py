...
def _create_engine_with_retry():
    ...
    # 验证连接，带重试；验证失败不阻断启动（engine 惰性连接，pool_pre_ping 会自动重连恢复）
    ...
    logger.error(
        f"Database connection failed after {MAX_RETRY_TIME}s: {last_error}. "
        "Serving with lazy engine; pool_pre_ping will recover connections when DB is back."
    )
    return engine
