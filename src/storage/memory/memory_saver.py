...
# 数据库连接超时时间（秒）。
# ⚠️ 平台 FaaS 启动窗口为 30 秒：应用必须在该窗口内完成 startup 并监听 5000 端口，
#    否则整个函数会被判定启动超时并 kill（2026-08-28 部署失败根因：
#    旧配置 15s × 2 次重试 = 最坏 31s，首次连接超时即触发平台 kill）。
# 现改为每次 5 秒、最多 3 次（含重试间隔最坏约 17 秒），为端口监听留足余量；
# 全部失败时退化为 MemorySaver，保证应用始终可以完成启动。
...
class MemoryManager:
    ...
    def _connect_with_retry(self, db_url: str) -> Optional[psycopg.Connection]:
        """带重试的数据库连接，每次 5 秒超时，共尝试 3 次，总耗时必须远小于平台 30 秒启动窗口"""
        last_error = None
    ...
    def get_checkpointer(self) -> BaseCheckpointSaver:
        ...
        try:
            ...
            # 注：AsyncConnectionPool 构造后由后台 worker 建立连接，不阻塞当前 startup
